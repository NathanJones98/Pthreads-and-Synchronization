...
int mutex_locks_size = (1 << 14);
pthread_mutex_t mutex_locks[(1 << 14)];
...
void *seed(void *vargp) {
  ...
  for (; from<=to; from++){ 
    ...
    for (j=0; j<SAMPLES_TO_COLLECT; j++){ 
      ...
      /*
      * Lookup -> returns null if list is empty
      * -> returns the locked element with value key if found
      * 
      * */
      if (!(s = h.lookup(key))){ //s is null
        pthread_mutex_lock(&mutex_locks[HASH_INDEX(key, mutex_locks_size-1)]);

        if (!(s = h.lookup(key))){ //s is still null
          // insert a new element for it into the hash table
          s = new sample(key);
          s->count++;
          h.insert(s);

          pthread_mutex_unlock(&mutex_locks[HASH_INDEX(key, mutex_locks_size-1)]);
        } else { // s is no longer null, increase counter
...
int main (int argc, char* argv[]){
  ...
  * */
  ...
  for(int p=0; p<mutex_locks_size; p++){
    mutex_locks[p] = PTHREAD_MUTEX_INITIALIZER;
  }
